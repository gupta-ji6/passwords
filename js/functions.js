...
$(document).ready(function () {
  ...
  $('#showPassword').on('click', function (e) {
    ...
    if (!User) {
      ...
      $info
        .html('Please enter a valid domain name (e.g., google)')
        .show()
        .delay(2000)
        .fadeOut('slow');
      ...
      $info
        .html('Please enter a valid numerical length for your password')
        .show()
        .delay(2000)
        .fadeOut('slow');
    } else if (!Phrase) {
      ...
      $info
        .html('Please enter your secret passphrase')
        .show()
        .delay(2000)
        .fadeOut('slow');
    } else {
      ...
      if (hostPattern.test(Domain) == false) {
        ...
        $info
          .html(
            'Please enter a valid domain name but without using any prefixes or suffixes. For instance, enter google and not google.com or www.google.com or accounts.google.com'
          )
          .show()
          .delay(2000)
          .fadeOut('slow');
...
function generate_password(User, Domain, Phrase, Len) {
  ...
  var salt =
    '$2a$10$' +
    hex_sha512(Domain + User + 'ed6abeb33d6191a6acdc7f55ea93e0e2').substr(
      0,
      21
    ) +
    '.';
  ...
  bcrypt.hashpw(
    ...
    function (result) {
      var hashed = b85_hash(
        result.slice(result.length - 31, result.length)
      ).substring(0, Len);
...
function b85_hash(s) {
  return ascii85(
    $.map(b64_sha512(s).split(''), function (val) {
...
var c = function (input, length, result) {
  ...
  for (i = 0; i < length; i += 4) {
    n =
      ((input[i] * 256 + input[i + 1]) * 256 + input[i + 2]) * 256 +
      input[i + 3];
    if (!n) {
      result.push('z');
    } else {
